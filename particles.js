...
(function () {
  ...
  function makePetal(x, y, burst, depth) {
    ...
    const sz  = burst ? 10 + Math.random()*26 : (7+Math.random()*18)*(0.5+depth*0.65);
    ...
    return {
      ...
      alpha: burst?0.95:0.28+Math.random()*0.55,
      alphaDecay: burst?0.006+Math.random()*0.006:0.0007,
  ...
  function makeFirefly() {
    return {
      ...
      blinkPeak:0.35+Math.random()*0.65,
  ...
  /* ── AMBIENT SPAWN ── */
  const SCALE=M?0.72:1;
  const C={petals:Math.floor(85*SCALE),dust:Math.floor(140*SCALE),orbs:Math.floor(22*SCALE),ff:Math.floor(18*SCALE)};
  ...
  function touchBurst(x,y) {
    const np=Math.floor(35*SCALE),nd=Math.floor(55*SCALE),no=Math.floor(10*SCALE);
